...
from autoprotocol.container import WellGroup, Well
...
def maxiprep(self, source_well, dest_well):
    """
    Perform maxi-prep on culture grown from source wells and resulting DNA
    aliquot will be placed in destination wells.
    ...
    .. code-block:: python
        ...
        p = Protocol()
        my_cell_plate = p.ref("my_cell_plate", None, "96-flat", discard = True)
        my_dna_tube = p.ref("my_dna_tube", None, "micro-2.0", storage = "cold_4")

        p.maxiprep(my_cell_plate.wells_from(0,4), my_dna_tube.well(0))
    ...
    ----------
    source_well : Well, WellGroup
      Well(s) containing bacteria to be maxi-prepped
    dest_well : Well, WellGroup
      Well(s) indicating final storage location(s) of resulting DNA
    ...
    self.instructions.append(Maxiprep(group_opts))
